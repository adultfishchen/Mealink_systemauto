...
function scheduleClearCronstyle(){
  schedule.scheduleJob("00 43 18 * * *", function(){
    MongoClient.connect(
      url,
...
function scheduleCronstyle(){
  schedule.scheduleJob("00 44 18 * * *", function(){
    MongoClient.connect(
      url,
      { useNewUrlParser: true, useUnifiedTopology: true },
      function (err, db) {
              ...
              if(result[a].number ===2){
                ...
                for (b = 0; b < userdouble.length; b++) {
                  if (userdouble[b].match === null) {
                    ...
                    var basevalue = [];
                    for (c = 0; c < userdouble[b].matchHabies.select.length; c++) {
                    ...
                    for (f = b + 1; f < userdouble.length; f++) {
                      ...
                      if (userdouble[f].match === null) {
                        compuser = userdouble[f];
                        var compvalue = [];
                        for (g = 0; g < userdouble[f].matchHabies.select.length; g++) {
                          var comvalue = userdouble[f].matchHabies.select[g] * 1;
              ...
              if (manyusers[i].match === null) {
                ...
                for (k = i + 1; k < manyusers.length; k++) {
                  ...
                  if (manyusers[k].match === null) {
                    ...
                    // console.log( "mcompuser: " + manyusers[k]._id);
                    var mcompvalue = [];
                    for (l = 0; l < manyusers[k].matchHabies.select.length; l++) {
                      var mcomvalue = manyusers[k].matchHabies.select[l] * 1;
                      // console.log(mcomvalue);
                ...
                mans.sort(function(a, b){return b-a});
                ...
                for(p=0; p< mans.length; p++){                    
                      if(mans[p]>0){
                        ...
                        // console.log("mcumusers:" + mcusermatches)
                          }                     
                        } 
                        dbo
                    .collection("users")
                    .updateOne(
                      { _id: mbaseuser._id },
                      { $set: { match: [mcusermatches[0],mcusermatches[1],mcusermatches[2]]} }
                    );
                  dbo
                    .collection("users")
                    .updateOne(
                      { _id: mcusermatches[0] },
                      { $set: { match: [ mbaseuser._id, mcusermatches[1], mcusermatches[2]]}}
                    );
                    dbo
                    .collection("users")
                    .updateOne(
                      { _id: mcusermatches[1] },
                      { $set: { match: [ mbaseuser._id, mcusermatches[0], mcusermatches[2]] }}
                    );
                    dbo
                    .collection("users")
                    .updateOne(
                      { _id: mcusermatches[2] },
                      { $set: { match: [ mbaseuser._id, mcusermatches[0], mcusermatches[1]]}}
                    );
                   }
